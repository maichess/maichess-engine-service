...
│   ├── EvalV2.scala                 ← Tier 4 eval: + king safety + pawn structure + rook bonuses + bishop pair + outposts + pawn threats + tempo (tapered)
│   ├── MultiPvSearch.scala          ← Trait: bestMoveAtDepth + extractPv (multi-PV surface for analysis)
...
│   ├── SearchV4.scala               ← Tier 4: SearchV3 + EvalV2 + draw detection + ply-adjusted/depth-preferred TT + analysis mode
...
- `EnhancedEval` (tier 4) → `chess/SearchV4` — `SearchV3` paired with `chess/EvalV2`, a richer positional evaluation: king safety (exposed kings penalised), pawn structure (doubled/isolated/passed pawns with rank-scaled bonuses), rook bonuses (open files, 7th rank), full piece mobility (knight/bishop mobility masks own-occupied squares, consistent with rook/queen), a bishop-pair bonus, **knight/bishop outposts** (relative ranks 4–6, pawn-defended and unreachable by an enemy pawn via `pawnSpanMask`), **pawn threats** (own non-pawn piece attacked by an enemy pawn, value-scaled), and a **tempo** bonus (constant, side-to-move). Most terms taper between opening and endgame via the game-phase factor (tempo does not — so a mirror-symmetric position evaluates to `Tempo`, and `evaluate(wtm) + evaluate(btm) == 2·Tempo` rather than negating exactly). SearchV4 also carries the strongest-bot search hardening (it is the engine under tier-5 `SearchV5`):
  - **Draw detection** — in-search repetition (a position recurring on the search path scores an immediate `0`, bounded by the half-move clock so only same-fifty-move-window positions are scanned) and the 50-move rule (`halfMoveClock >= 100`, suppressed while in check so it cannot override checkmate). Path-dependent, so draw scores are **never** written to the TT. Only repetitions the search itself creates are seen (the engine is handed a bare FEN with no prior game history) — enough to stop repeating in a won position or to claim a perpetual when worse.
  - **Ply-adjusted mate scores** across the TT (`toTT`/`fromTT`) so a mate's distance-to-mate stays correct regardless of the ply it was stored at vs probed from.
  - **Depth-preferred TT replacement** — a deeper entry for a *different* position survives an index collision; the same position or an empty slot always refreshes. (Each search owns a fresh TT, so no generational aging.)
  - **Analysis mode** — `bestMoveAtDepth` (multi-PV analysis) sets an `analysisMode` flag that disables NMP and LMR, so the score of *every* line is a faithful evaluation rather than a fast best-move lower bound. `bestMove` (play) leaves them on.
- `Knowledge` (tier 5) → `service/SearchV5` — `SearchV4` augmented with a **Polyglot opening book** (`chess/OpeningBook`, binary `.bin` format, `PolyglotZobrist` for board→key) and **endgame tablebase probing** via `service/clients/TablebaseClient` (Syzygy; HTTP query to the Lichess endpoint for positions with ≤7 pieces). The book plays the **highest-weight** move deterministically (max strength, no weighted-random sampling); tablebase answers are used when the remaining piece count is low. Opening book keys use `chess/PolyglotZobrist` (distinct from the search `Zobrist`).
