...
// Separate from the engine's incremental Zobrist (Zobrist.scala). Used solely
// to compute the 8-byte key that indexes a Polyglot .bin opening book.
//
// The Random64 table below MUST be the literal published constants from the
// Polyglot reference implementation. Any deviation produces keys that do not
// match real .bin files.
//
// Index layout (781 total entries):
//   0..767  pieces — 12 pieces × 64 squares, Polyglot piece ordering:
//              0=bpawn  1=wpawn  2=bknight  3=wknight
//              4=bbishop 5=wbishop 6=brook   7=wrook
//              8=bqueen  9=wqueen 10=bking  11=wking
//   768..771 castling — independent slots: 768=WK, 769=WQ, 770=BK, 771=BQ.
//              XOR each slot ONLY IF that right is still held.
//   772..779 en-passant file — XOR Random64[772 + file(epSq)] ONLY IF a pawn
//              of the side-to-move can actually capture EP on this turn.
//   780     side to move — XOR if and only if White is to move.
@SuppressWarnings(Array("org.wartremover.warts.Var"))
object PolyglotZobrist:

  // Polyglot piece indices for engine piece tuples (color, type).
  // Engine: color 0=White, 1=Black; PType 0..5 = Pawn..King.
  // Polyglot: 2 * (PType) + (1 - color)  →  WPawn=1, BPawn=0, WKing=11, BKing=10.
  private inline def polyPieceIdx(color: Int, ptype: Int): Int =
    2 * ptype + (1 - color)
  ...
  def key(pos: Position): Long =
    ...
    while bb != 0L do
      ...
      val p  = pos.mailbox(sq)
      val color = Pieces.colorOf(Piece(p))
      val ptype = Pieces.typeOf(Piece(p))
      k ^= Random64(polyPieceIdx(color, ptype) * 64 + sq)
      bb &= bb - 1L

    // ── Castling rights — one XOR per right that is still held ────────────
    if (pos.castlingRights & Castling.WK) != 0 then k ^= Random64(768)
    if (pos.castlingRights & Castling.WQ) != 0 then k ^= Random64(769)
    if (pos.castlingRights & Castling.BK) != 0 then k ^= Random64(770)
    if (pos.castlingRights & Castling.BQ) != 0 then k ^= Random64(771)

    // ── En passant — only if a side-to-move pawn can actually capture ─────
    val ep = pos.epSquare
    if ep != Sq.None.toInt then
      val stm     = pos.sideToMove
      val pawnsBB = pos.pieceBB(stm, PType.Pawn)
      // Squares from which a pawn of color `stm` attacks `ep` equals the
      // squares attacked by a pawn of the opposite color sitting on `ep`.
      val attackers = Attacks.pawnAttacks(stm ^ 1)(ep) & pawnsBB
      if attackers != 0L then
        k ^= Random64(772 + (ep & 7))

    // ── Side to move ──────────────────────────────────────────────────────
    if pos.sideToMove == Col.White then k ^= Random64(780)

    k

  // ── Random64 table (Polyglot literal constants — 781 entries) ──────────────
  // Verified against three known starting/early-game keys; do not modify.
  private val Random64: Array[Long] = Array(
