...
// SearchV3 — SearchV2 (PVS + LMR + NMP) plus five move-ordering and
// search-control refinements:
//
//   - Killer moves: two quiet moves per ply that recently caused a beta
//     cutoff are tried right after the TT move and captures.
//   - History heuristic: a per-(side, from, to) score, bumped on cutoffs and
//     lightly penalised on quiet moves that failed to cut, orders the
//     remaining quiet moves; values are clamped to ±HistCap.
//   - Static Exchange Evaluation (SEE): captures are scored by the full
//     exchange sequence on the target square; losing captures sort below
//     quiet moves.
//   - Aspiration windows: each iterative-deepening iteration past depth 3
//     starts with a narrow window around the previous score and re-searches
//     with a widened window on a fail-high/low.
//   - Check extensions: a node where the side to move is in check is searched
//     one ply deeper, capped by ply so a perpetual-check line cannot blow up
//     the recursion.
//
// Like Search/SearchV2, SearchV3 is a final class — each request gets a fresh
...
final class SearchV3 extends MultiPvSearch:
  ...
  private val MV = Array(100, 320, 330, 500, 900, 20000, 0)   // piece values; index 6 guards NoPiece
  private val NullMoveR = 3
  private val HistCap   = 1 << 20
  private val MaxExtPly = 96               // no check extensions past this ply (recursion guard)
  ...
  // One iterative-deepening iteration. Below depth 4 (or with no usable prior
  // score) the search uses a full window; otherwise it brackets the previous
  // score and re-searches with a doubling delta until the result lands inside.
  ...
  // Static Exchange Evaluation for a capture move: the net material the side to
  // move wins (positive) or loses (negative) if the exchange on the target
  // square is played out with least-valuable-attacker-first recaptures.
  ...
  // All pieces of either colour attacking `sq` given occupancy `occ`. Sliders
  // are recomputed against `occ` so x-rayed attackers appear once the blocker
  // in front of them has been removed.
